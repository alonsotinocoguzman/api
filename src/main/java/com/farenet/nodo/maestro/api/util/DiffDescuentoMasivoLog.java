...
public class DiffDescuentoMasivoLog implements Runnable {

	private String target;
	private DescuentoMasivo descuento1;
	private DescuentoMasivo descuento2;
	private TriggerTaskFactory trigger;
	private String username;



	public DiffDescuentoMasivoLog(String target, DescuentoMasivo descuento1, DescuentoMasivo descuento2, String username, TriggerTaskFactory trigger) {
		this.target = target;
	...
	public void run() {
		...
		if(descuento1==null)
		{
			...
			for(DescuentoMasivoDetalle dd2 : descuento2.getDescuentomasivodetalles())
			{
			...
			for(DescuentoMasivoDetalle dd2 : descuento2.getDescuentomasivodetalles())
			{
				...
				for(DescuentoMasivoDetalle dd1 : descuento1.getDescuentomasivodetalles())
				{
		...
		try {
			trigger.launch( username, target, 	"DIFERENCIAS DESCUENTO MASIVO", mapper.writeValueAsString(diff));
