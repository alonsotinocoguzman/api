...
public class SendMailOfisis {
	...
	SimpleDateFormat dtfPeru= new SimpleDateFormat(UIUtils.FORMATO_FECHA_PERU);
	 ...
	 public void execute(String planta, List<String> detalle, long cantidad,String correo,Date fechaIni,Date fechaFin) {
		 
		 StringBuffer detal = new StringBuffer();
		 for(int i = 0; i<detalle.size();i++) {
			 detal.append(detalle.get(i).toString() + " | ");
		 }
		 
		 trigger.launch("", "mail.ofisis.begin", "ENVIAR POR CORREO OFISIS", "Planta:" + planta + " | No se enviaron "+detalle.size()+" de " + cantidad  + "DETALLE : "+ detal.toString());
		...
		StringBuffer cadenaHTML = new StringBuffer(
				...
				+ "Tienes " + (cantidad-detalle.size()) + "que se enviaron a Ofisis." 
				+ "</span>\n" + 
				...
				eventStr.append("<b><big>Planta : ").append(planta).append("</big></b></p>\n");
				eventStr.append("<p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 15px;'>");
				eventStr.append("<b>Fechas desde : </b>").append(dtfPeru.format(fechaIni)).append("<b> | hasta : </b>").append(dtfPeru.format(fechaFin)).append("</p>\n");
				eventStr.append("<p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 15px;'>");
				eventStr.append("<b>Se enviaron : </b>").append(cantidad-detalle.size()).append(" de "+cantidad+ " comprobantes. \n").append("</p>\n");
				eventStr.append("<p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 15px;'>"+
				"<b>No se enviaron : </b></p>\n");
				
				for(String log : detalle)
				{
					
					eventStr.append("<p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 15px;'>");
					eventStr.append("<b>Comprobante : </b>").append(log).append("</p>\n");
		...
		try{
			 ...
			 trigger.launch("", "mail.ofisis.ok", "ENVIAR POR CORREO OFISIS", "Planta:" + planta + " | No se enviaron "+detalle.size()+" de " + cantidad  + "DETALLE : "+ detal.toString());
		     
		}catch(Exception e)
		{
			e.printStackTrace();
			trigger.launch("", "mail.ofisis.error", "ERROR AL ENVIAR",e.getMessage());
			trigger.launch("", "mail.ofisis.error", "ENVIAR POR CORREO OFISIS", "Planta:" + planta + " | No se enviaron "+detalle.size()+" de " + cantidad  + "DETALLE : "+ detal.toString());
